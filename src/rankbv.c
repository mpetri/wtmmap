...
rankbv_t*
rankbv_init(size_t n,uint32_t f)
{
...
rankbv_load(FILE* f)
{
    size_t bytes;
    /* read space */
    if (fread(&bytes,sizeof(size_t),1,f) != 1) {
        fprintf(stderr,"ERROR LOADING RANKBV\n");
    }

    /* read the bv */
    void* mem = rankbv_safecalloc(bytes);
    if (fread(mem,bytes,1,f) != 1) {
        fprintf(stderr,"ERROR LOADING RANKBV\n");
    }
